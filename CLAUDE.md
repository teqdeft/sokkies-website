...
OFFERTEFORMULIER (/offerte/) — NIEUW GRAVITY FORM, STAP ONTHOUDEN NA
  ...
  POSTCODE.EU ERUIT HALEN: GEPROBEERD EN TERUGGEDRAAID (2026-09-22). Op
  verzoek is Nederland even volledig naar Google Address Validation gegaan en
  is alle Postcode.eu-code verwijderd; datzelfde verzoek werd daarna
  teruggedraaid (commit 24a430e met git revert ongedaan gemaakt). De
  verdeling is dus weer: Nederland via Postcode.eu, al het overige via Google.
  WAAROM DIT NIET NOG EEN KEER BLIND GEPROBEERD MOET WORDEN — gemeten, niet
  aangenomen: Google leidt een straat NIET af uit een postcode. Vier
  schrijfwijzen getest (postcode in het eigen veld, mét spatie, alles op één
  regel, huisnummer vóór de postcode) en alle vier geven OTHER zonder
  route-onderdeel; pas mét straatnaam komt er PREMISE uit. Nederlandse
  bezoekers zouden dus ook hun straat moeten typen, terwijl Postcode.eu het
  hele adres uit postcode + huisnummer vult. Bovendien verdwijnt de harde
  "bestaat niet": 5211AB/12 geeft bij Google precies hetzelfde antwoord als
  een adres dat wél bestaat, waar Postcode.eu een 404 geeft.
  WAT ER UIT DIE RONDE IS BLIJVEN HANGEN, want het is een echt gedrag van
  Google: bij een NEDERLANDSE postcode die niet bij de straat hoort VERVANGT
  hij hem en zet er CONFIRMED bij (2012ES + Kalverstraat 30 -> 1012 PD
  Amsterdam). Buiten Nederland doet hij dat niet: daar echoot hij de postcode
  terug met UNCONFIRMED_BUT_PLAUSIBLE (nagemeten met een Berlijnse straat op
  een postcode uit Wiesbaden, Antwerpen op 1000, Parijs op 13116 en Manchester
  op SW1A2AA). Daarom is de controle op de bevestiging van de postcode genoeg
  ...
  POSTCODE.EU IS NU ECHT ALLEEN NEDERLAND (2026-09-22, besluit Kulwant met een
  schermafbeelding van een Franse postcode met straatsuggesties: "but it is
  working for postcode.eu"). De straatsuggesties die eerder die dag waren
  teruggebouwd zijn weer weg (commit 7677f52 met git revert ongedaan gemaakt),
  want daarvoor werd Postcode.eu ook op buitenlandse postcodes aangeroepen.
  De verdeling is nu zonder uitzonderingen:
    Nederland       Postcode.eu (PDOK alleen als er geen sleutel is)
    alle andere     Google Address Validation
  WAT DIT KOST, eerlijk vastgelegd:
  (1) GEEN KEUZELIJST MEER buiten Nederland. De bezoeker typt zijn straat in
  een gewoon veld. Google kan zo'n lijst niet leveren — Address Validation
  keurt alleen een compleet adres en Places Autocomplete heeft eerst getypte
  letters nodig. Zie de eerdere notitie voor de details.
  (2) HET VERENIGD KONINKRIJK MOET WEER TYPEN. Met de lijst erbij vulde de code
  de straat zelf in zodra een postcode er maar één opleverde; dat viel weg.
  (3) GOOGLE KENT NIET ALLE FRANSE STRATEN. Uit de schermafbeelding zelf:
  "Allée des Pervenches" in 38240 staat wél in de lijst van Postcode.eu, maar
  Google geeft er OTHER op met straat én postcode UNCONFIRMED_BUT_PLAUSIBLE —
  ook zonder accenten en met het huisnummer vooraan. Dat adres wordt dus
  geweigerd terwijl het bestaat. Reken erop dat dit vaker gebeurt in Frankrijk.
  DE CONTROLES BLIJVEN ONGEWIJZIGD: een straat die niet bij de postcode hoort
  ...
  VERVOLG (verzoek Kulwant): 2px lucht tussen de thumbs en de scrollbar, maar
  ALLEEN als de kolom echt scrolt. CSS kan niet zien of een element scrolt,
  dus een kleine IIFE in custom.js zet de klasse heeft-scrollbar op
  .prod-thumbs (verticaal gemeten, dus de horizontale mobiele rij krijgt hem
  vanzelf niet) en hermeet bij resize en na window.load.
  DE PADDING HOORT OP DE CONTAINER, niet op de thumb. Eerst stond hij op
  .prod-thumb, maar dat is een knop met overflow:hidden en een foto op 100%:
  dan wordt alleen de FOTO smaller en blijft de knop even breed, dus er komt
  geen lucht bij de scrollbar. Op .prod-thumbs werkt het wel.
  GEMETEN met scrollbar: container padding 2px, thumb 98px (was 100), foto
  94px, afstand van de thumbrand tot de kolomrand 12px (2px padding + 10px
  scrollbar). Zonder scrollbar: geen klasse, padding 0, thumb 110, foto 106.
